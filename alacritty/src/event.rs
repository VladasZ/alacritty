...
use crate::display::{Display, Preedit, SizeInfo, TabHit};
...
impl ApplicationHandler<Event> for Processor {
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: Event) {
        ...
        match (payload, window_id) {
            ...
            // Capture the focused window, or the first one if none is focused.
            #[cfg(unix)]
            (EventType::Screenshot, _) => {
                let id = self
                    .windows
                    .iter()
                    .find(|(_, window_context)| window_context.focused())
                    .map(|(id, _)| *id)
...
impl<'a, N: Notify + 'a, T: EventListener> input::ActionContext<T> for ActionContext<'a, N, T> {
    ...
    fn close_window(&mut self) {
        let _ = self.event_proxy.send_event(Event::new(
            EventType::Tab(TabAction::RequestClose),
            self.display.window.id(),
        ));
    }
